...
class FSIM_base(nn.Module):
    ...
    def __init__(self):
        ...
        self.fo = (1.0 / (minWaveLength * pt.pow(mult, (pt.arange(0, self.nscale, dtype=pt.float64))))).unsqueeze(
    ...
    def calculate_gradient_sim(self, gradientMap1, gradientMap2):
        ...
        gradientSimMatrix = (2 * gradientMap1 * gradientMap2 + self.T2) / (
                    pt.pow(gradientMap1, 2) + pt.pow(gradientMap2, 2) + self.T2)
    ...
    def phasecong2(self, img):
        ...
        imagefft_repeat = pt.cat(self.nscale * self.norient * [imagefft], dim=1).view(-1, self.nscale, self.norient,
                                                                                      rows, cols, 2)
...
class FSIMc(FSIM_base, nn.Module):
    ...
    def forward(self, imgr, imgd):
        ...
        gradientSimMatrix = self.calculate_gradient_sim(gradientMap1, gradientMap2)
        gradientSimMatrix = gradientSimMatrix.view(PCSimMatrix.size())
        FSIMc = self.calculate_fsimc(I1.squeeze(), Q1.squeeze(), I2.squeeze(), Q2.squeeze(), gradientSimMatrix,
                                     PCSimMatrix, PCm)
